...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        metadata = 'Not Exists' if (val:=user_dict.get('metadata', config_dict.get('METADATA_KEY', ''))) == '' else val
        buttons.ibutton(f"{'✅️' if metadata != 'Not Exists' else ''} Metadata", f"userset {user_id} metadata")

        attachment = 'Not Exists' if (val:=user_dict.get('attachment', config_dict.get('ATTACHMENT_URL', ''))) == '' else val
        ...
        elif key in ['lprefix', 'lremname', 'lsuffix', 'lcaption', 'ldump', 'metadata', 'attachment']:
        ...
        elif key in ['mprefix', 'mremname', 'msuffix', 'metadata', 'attachment']:
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'yt_opt', 'metadata', 'attachment'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'mprefix', 'msuffix', 'mremname', 'attachment', 'metadata']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlsuffix', 'dlremname', 'dlcaption', 'dldump', 'dattachment', 'dmetadata']:
        handler_dict[user_id] = False
        await query.answer()
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['dmprefix', 'dmsuffix', 'dmremname', 'duser_tds', 'dattachment', 'dmetadata']:
        handler_dict[user_id] = False
        await query.answer()
